...
namespace FengShuiNumber.AutoGenerate
{
    public static class AutoGenerate
    {
        ...
        public static string AutoGenerateCode(int length, bool isDigit =false, bool isSpecialChar = false, bool isUpper = false)
        {
            const string alphanumericCharacters = "abcdefghijklmnopqrstuvwxyz";
            if(isUpper)
                return GetRandomString(length, alphanumericCharacters, isDigit, isSpecialChar).ToUpper();
            return GetRandomString(length, alphanumericCharacters, isDigit, isSpecialChar);
        }

        public static string AutoGenerateUpperCodeNumber(int length)
        {
            const string alphanumericCharacters = "abcdefghijklmnopqrstuvwxyz";
            return GetRandomString(length, alphanumericCharacters, true, false).ToUpper();
        }

        public static string AutoGenerateNumber(int length)
        {
            const string characters = "0123456789";
            return GetRandomString(length, characters, false, false);
        }
    }
}
